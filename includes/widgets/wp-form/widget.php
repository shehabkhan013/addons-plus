...
class Wp_Form extends Widget_Base {
	...
	public function get_style_depends() {
		return [ 'addons-plus-wp-forms-style' ];
	}
	...
	protected function _register_controls() {
		$this->start_controls_section(
			'_section_wpforms',
			[
				'label' => Initializations::is_wpforms_activated() ? __( 'WPForms', 'addons-plus' ) : __( 'Missing Notice',
					'addons-plus' ),
				'tab'   => Controls_Manager::TAB_CONTENT,
			]
		);

		if ( ! Initializations::is_wpforms_activated() ) {
			$this->add_control(
				...
				[
					...
					'raw'             => sprintf(
						...
						'<a href="' . esc_url( admin_url( 'plugin-install.php?s=WPForms&tab=search&type=term' ) ) . '" target="_blank" rel="noopener">' . __( "WPForms", 'addons-plus' ) . '</a>',
			...
			$this->add_control(
				...
				[
					'type' => Controls_Manager::RAW_HTML,
					'raw'  => '<a href="' . esc_url( admin_url( 'plugin-install.php?s=WPForms&tab=search&type=term' ) ) . '" target="_blank" rel="noopener">' . __( "Click to install or activate WPForms", "addons-plus" ) . '</a>',
				]
			);
		...
		$this->add_control(
			...
			[
				...
				'label_block' => true,
				'options'     => [ '' => __( 'Select a WPForms', 'addons-plus' ) ] + Initializations::get_wpforms(),
			]
		);
		$this->add_control(
			'html_class',
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .adp-wpForm-form-wrap',
			]
		);
		$this->add_control(
			'border_radius',
			[
				...
				'size_units' => [ 'px', '%', 'em' ],
				'selectors'  => [
					'{{WRAPPER}} .adp-wpForm-form-wrap' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		$this->add_control(
			...
			[
				...
				'size_units' => [ 'px', '%', 'em' ],
				'selectors'  => [
					'{{WRAPPER}} .adp-wpForm-form-wrap' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'size_units'         => [ 'px' ],
				'selectors'          => [
					'{{WRAPPER}} .adp-wpForm-info h5' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				]
			]
		);
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .adp-wpForm-info h5',
			]
		);
		$this->add_control(
			'subheading_margin',
			[
				...
				'allowed_dimensions' => [ 'top', 'bottom' ],
				'selectors'          => [
					'{{WRAPPER}} .adp-wpForm-info p' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				]
			]
		);
		$this->add_group_control(
			...
			[
				...
				'scheme'   => Scheme_Typography::TYPOGRAPHY_1,
				'selector' => '{{WRAPPER}} .adp-wpForm-info p',
			]
		);
		$this->end_controls_section();
		$this->start_controls_section(
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors'  => [
					'{{WRAPPER}} .wpforms-field input:not(.wpforms-submit)'    => 'width: {{SIZE}}{{UNIT}};',
					'{{WRAPPER}} .wpforms-field textarea:not(.wpforms-submit)' => 'width: {{SIZE}}{{UNIT}};',
	...
	protected function render() {
		...
		if ( ! empty( $settings['form_id'] ) ) {
			?>
            <div class="adp-wpForm-form-wrap">
                <div class="adp-wpForm-info" style="text-align: <?php echo esc_attr( $settings['ingo_alignment'] ); ?>">
                    <h5><?php echo esc_html__( $settings['contact_title'], 'addons-plus' ) ?></h5>
                    <p><?php echo esc_html__( $settings['info'], 'addons-plus' ) ?></p>
                </div>
				<?php
				$_class = 'adp-wpForm-form ' . sanitize_html_class( $settings['html_class'] );
				echo do_shortcode( '[wpforms id="' . $settings['form_id'] . '" html_class="' . $_class . '"]' );
				?>
            </div>
			<?php
		}
