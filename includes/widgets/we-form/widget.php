 ...
 * Elementor weForms
 *
 * Elementor widget for weForms.
 *
 * @since 1.0.0
 */
class We_Form extends Widget_Base {
	...
	protected function _register_controls() {
		...
		if ( ! Initializations::is_weforms_activated() ) {
			$this->add_control(
				...
				[
					...
					'raw'             => sprintf(
						...
						'<a href="' . esc_url( admin_url( 'plugin-install.php?s=weForms&tab=search&type=term' ) ) . '" target="_blank" rel="noopener">' . __( "weForms", "addons-plus" ) . '</a>',
			...
			$this->add_control(
				...
				[
					'type' => Controls_Manager::RAW_HTML,
					'raw'  => '<a href="' . esc_url( admin_url( 'plugin-install.php?s=weForms&tab=search&type=term' ) ) . '" target="_blank" rel="noopener">' . __( "Click to install or activate weForms", "addons-plus" ) . '</a>',
				]
			);
		...
		$this->add_control(
			...
			[
				...
				'label_block' => true,
				'options'     => [ '' => __( 'Select a WeForm', 'addons-plus' ) ] + Initializations::get_we_forms(),
			]
		);
		$this->add_control(
			'html_class',
		...
		$this->add_group_control(
			...
			[
				...
				'selector'  => '{{WRAPPER}} .wpuf-form-sub-label',
				'scheme'    => Scheme_Typography::TYPOGRAPHY_3,
				'condition' => [
					'sub_label' => 'block'
				],
