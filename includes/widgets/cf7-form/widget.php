 ...
 * @since 1.0.0
 */
class Cf7_Form extends Widget_Base {
	...
	protected function _register_controls() {
		...
		if ( ! Initializations::is_cf7_activated() ) {
			$this->add_control(
				...
				[
					...
					'raw'             => sprintf(
						...
						. '" target="_blank" rel="noopener">' . __( "Contact Form 7", "addons-plus" ) . '</a>',
			...
			$this->add_control(
				...
				[
					'type' => Controls_Manager::RAW_HTML,
					'raw'  => '<a href="' . esc_url( admin_url( 'plugin-install.php?s=Contact+Form+7&tab=search&type=term' ) ) . '" target="_blank" rel="noopener">' . __( "Click to install or activate Contact Form 7", "addons-plus" ) . '</a>',
				]
			);
		...
		$this->add_control(
			...
			[
				...
				'label_block' => true,
				'options'     => [ '' => __( 'Select a CF7', 'addons-plus' ) ] + Initializations::get_cf7_forms(),
			]
		);
		$this->add_control(
			'html_class',
